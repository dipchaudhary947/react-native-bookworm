...
router.get("/user", protectRoute, async (req, res) => {
    try {
        const books = await Book.find({auther: req.user._id}).sort({createdAt: -1});
        res.json(books);
    } catch (error) {
...
router.delete("/:id", protectRoute, async (req, res) => {
    try {
        ...
        if(book.auther.toString() !== req.user._id.toString()) return res.status(401).json({message:"Unauthorized"});
